...
if (isset($_GET['txtID'])) {
    ...
    $sentencia = $conexion->prepare("SELECT imagen FROM proveedores WHERE id=:id");
    $sentencia->bindParam(":id", $txtID);
    $sentencia->execute();
    $registros_imagen = $sentencia->fetch(PDO::FETCH_ASSOC);

    if (!empty($registros_imagen["imagen"])) {
        $ruta_imagen = "../../../assets/img/about/" . $registros_imagen["imagen"];
    ...
    $sentencia = $conexion->prepare("DELETE FROM proveedores WHERE id=:id");
    $sentencia->bindParam(":id", $txtID);
    $sentencia->execute();
}
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Tipo_Proveedor</th>
                ...
                <tbody>
                    <?php foreach ($lista_proveedores as $registros) { ?>
                        <tr>
                            ...
                            <td scope="col"><?php echo $registros['tipo_proveedor']; ?></td>
